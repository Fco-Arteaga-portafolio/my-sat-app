...
import { regimenFiscal, usoCFDI, formaPago, metodoPago, cat } from '../../utils/catalogosSat'
  ...
  <>
    ...
    <Descriptions bordered column={2} size="small">
      ...
      {parseada.formaPago && (
        <Descriptions.Item label="Forma Pago">
          {cat(formaPago, parseada.formaPago)}
        </Descriptions.Item>
      )}
      {parseada.metodoPago && (
        <Descriptions.Item label="Método Pago">
          {cat(metodoPago, parseada.metodoPago)}
        </Descriptions.Item>
      )}
    ...
    <Descriptions bordered column={2} size="small">
      ...
      <Descriptions.Item label="Régimen Fiscal">
        {cat(regimenFiscal, parseada.regimenFiscal)}
      </Descriptions.Item>
    </Descriptions>
    ...
    <Descriptions bordered column={2} size="small">
      ...
      {parseada.descuento ? (
        <Descriptions.Item label="Descuento">{fmt(parseada.descuento)}</Descriptions.Item>
      ) : null}
      {parseada.totalImpuestosTrasladados ? (
        <Descriptions.Item label="IVA Trasladado">
          {fmt(parseada.totalImpuestosTrasladados)}
        </Descriptions.Item>
      ) : null}
      {parseada.totalImpuestosRetenidos ? (
        <Descriptions.Item label="Impuestos Retenidos">
          {fmt(parseada.totalImpuestosRetenidos)}
        </Descriptions.Item>
      ) : null}
      <Descriptions.Item label="Total" span={2}>
        <strong>{fmt(parseada.total)}</strong>
      </Descriptions.Item>
    </Descriptions>
    ...
    {parseada.complementoNomina && (
      <>
        ...
        <Descriptions bordered column={2} size="small">
          <Descriptions.Item label="Tipo Nómina">
            {parseada.complementoNomina.tipoNomina === 'O' ? 'Ordinaria' : 'Extraordinaria'}
          </Descriptions.Item>
          <Descriptions.Item label="Fecha Pago">
            {parseada.complementoNomina.fechaPago}
          </Descriptions.Item>
          <Descriptions.Item label="Período">
            {parseada.complementoNomina.fechaInicialPago} -{' '}
            {parseada.complementoNomina.fechaFinalPago}
          </Descriptions.Item>
          <Descriptions.Item label="Días Pagados">
            {parseada.complementoNomina.numDiasPagados}
          </Descriptions.Item>
          <Descriptions.Item label="Total Percepciones">
            {fmt(parseada.complementoNomina.totalPercepciones)}
          </Descriptions.Item>
          <Descriptions.Item label="Total Deducciones">
            {fmt(parseada.complementoNomina.totalDeducciones)}
          </Descriptions.Item>
        </Descriptions>
      </>
    )}
